...
const DEFAULT_BACKING_ALPHA = 0;
const CAPTURE_BACKING_ALPHA = 0;
 ...
 */
const scaleFactorVector = new vec3(scaleFactor + magicScalar,
...
export class ContainerFrame extends BaseScriptComponent {
  ...
  @ui.group_end
  @ui.separator
  private squeezeAmount = this.border * 0;
  ...
  private interactableHoverOpacity: number = 0;
  ...
  onAwake() {
      ...
      this.followButton = new LabeledPinchButton({
          ...
          toggle: true,
          triggerColor: new vec4(0, 0, 0, 0),
      });
      this.followButton.setIconScale(new vec2(0, 0));
     ...
     */
      if (!this.worldCursor) {
          const sikRootSceneObject = SIK.InteractionManager.getInteractorsByType(InteractorInputType.RightHand)[0]
              .sceneObject.getParent()
              .getParent();
          const interactorCursorsSceneObject = findSceneObjectByName(sikRootSceneObject,
              'InteractorCursors');
          this.worldCursor = interactorCursorsSceneObject.getComponent(CursorController.getTypeName());
      }
     ...
     */
      ...
      this.unSubscribeList.push(this.parentHoverBehavior.onHoverStart.add((e: InteractorEvent) => {
          this.cursorHandler.setCursor(SIK.CursorController.getCursorByInteractor(e.interactor));
      ...
      this.unSubscribeList.push(this.parentHoverBehavior.onHoverUpdate.add((e: InteractorEvent) => {
      ...
      this.unSubscribeList.push(this.interactable.onHoverUpdate.add((event: InteractorEvent) => {
      ...
      this.unSubscribeList.push(this.interactable.onTriggerStart((e: InteractorEvent) => {
          const targetObject = e?.target.sceneObject;
          let targetParent = targetObject;
  ...
  private scaleFrame = () => {
      ...
      this.material.mainPass.rawScale = new vec2(this.targetScaleCache.x + this.constantPadding.x,
          this.targetScaleCache.y + this.constantPadding.y);

      const fullScale = new vec2(this.targetScaleCache.x + this.constantPadding.x + doubleMargin,
      ...
      this.material.mainPass.originalScale = new vec2(this.originalScale.x + this.currentBorder,
      ...
      this.renderMeshVisual.mainMaterial.mainPass.frustumCullMin = new vec3(this.colliderShape.size.x * -0.5 - this.frustumBuffer,
          this.colliderShape.size.y * -0.5 - this.frustumBuffer,
          this.colliderShape.size.z * -0.5);

      this.renderMeshVisual.mainMaterial.mainPass.frustumCullMax = new vec3(this.colliderShape.size.x * 0.5 + this.frustumBuffer,
          this.colliderShape.size.y * 0.5 + this.frustumBuffer,
          this.colliderShape.size.z * 0.5);
  ...
  private scaleAndPositionButtons = () => {
      ...
      // CORNER POSITIONING
      this.closeButton.transform.setLocalPosition(new vec3(-halfFrameWidth - buttonOffset,
          halfFrameHeight + buttonOffset,
          0.1));

      this.followButton.transform.setLocalPosition(new vec3(500 + 50,
          halfFrameHeight + buttonOffset,
          0.1));
  };
  ...
  setYAlways = (yAlways: boolean) => {
      this.setBillboarding(this.useBillboarding,
          this.xOnTranslate,
          this.xAlways,
          this.yOnTranslate,
          yAlways);
  ...
  setUseBillboarding = (useBillboarding: boolean): void => {
      this.setBillboarding(useBillboarding,
          this.xOnTranslate,
          this.xAlways,
          this.yOnTranslate,
          this.yAlways);
  ...
  get totalInnerSize(): vec2 {
      return new vec2(this.innerSize.x + this.constantPadding.x,
   ...
   * @param targetOpacity
   */
  tweenOpacity = (currentOpacity: number,
  ...
  private tweenMarginSqueeze = (currentSqueeze: number,
